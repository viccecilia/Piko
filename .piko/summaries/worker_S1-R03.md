...
## Scope
- Allowed files touched: apps/api/routes/workflow.py; tests/test_stage_1.py
...
## Changes
- Modified files: apps/api/routes/workflow.py; tests/test_stage_1.py
- Added files: See modified files list when new module/fixture/test was required for this round.
- Deleted files: None.
- Behavioral changes: Added POST /workflow/article/run with required player_question while preserving old endpoint.
...
## Sample Output
`json
{"pipeline_state":{"player_question":"crash on startup"},"publish_decision":{"value":"verified_candidate"}}
`
