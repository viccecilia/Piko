...
## Scope
- Allowed files touched: apps/api/routes/workflow.py; fixtures/verification/*.json; tests/test_stage_4.py
...
## Changes
- Modified files: apps/api/routes/workflow.py; fixtures/verification/*.json; tests/test_stage_4.py
- Added files: See modified files list when new module/fixture/test was required for this round.
- Deleted files: None.
- Behavioral changes: Added POST /workflow/article/verify and local verification fixture notes.
...
## Sample Output
`json
{"status":"fail","checks":[{"name":"evidence_source_trace","status":"fail"}]}
`
